...
async def get_user_by_login(db, login: str) -> Optional[User]:
    """
    CRUD async GET that returns the user with given id
    :param db that executes the statement
...
async def create_user(db, user: SchemaUserWithPassword) -> Optional[User]:
    """
...
async def change_user(db, user_id: int, user: SchemaUserOptional) -> Optional[User]:
    """
    CRUD async POST that creates and returns the user
    :param db that executes the statement
    :param user_id: a required parameter that identifies the user
...
async def get_user_by_token(db, token: str) -> Optional[User]:
    """
